...
func (c *cli) triggerStack(basePath string) {
	...
	if !c.parsedArgs.Experimental.Trigger.Recursive {
		...
		var err error
		stacksReport, err := c.listStacks(false, cloudstack.AnyTarget, cloud.StatusFilters{})
		if err != nil {
...
func parseDeploymentStatusFilter(filterStr string) deployment.FilterStatus {
	...
	if err != nil {
		fatalWithDetails(err, "failed to parse deployment filter")
	}
	return filter
}
...
func parseDriftStatusFilter(filterStr string) drift.FilterStatus {
	...
	if err != nil {
		fatalWithDetails(err, "failed to parse drift filter")
	}
	return filter
}

func (c *cli) printRuntimeEnv() {
	report, err := c.listStacks(c.parsedArgs.Changed, cloudstack.AnyTarget, cloud.NoStackFilter())
	if err != nil {
...
func (c *cli) printRunOrder(friendlyFmt bool) {
	...
	stacks, err := c.computeSelectedStacks(false, "", cloud.NoStackFilter())
	if err != nil {
		fatalWithDetails(err, "computing selected stacks")
	}
...
func (c *cli) ensureStackID() {
	report, err := c.listStacks(false, cloudstack.AnyTarget, cloud.NoStackFilter())
	if err != nil {
		fatalWithDetails(err, "listing stacks")
	}
